    ...
    <div className="mx-auto w-full pb-20">
      <div className="pt-8 relative w-screen -left-4 scroll-px-6 flex snap-x snap-proximity gap-6 overflow-x-auto overflow-y-hidden scroll-smooth scrollbar-none px-6">
        { booksWithAverage.map((book) => {
          ...
          return (
            <div className="group flex flex-col justify-between w-36 shrink-0 snap-start sm:w-44 md:w-54" key={book.id}>
              <div>
                ...
                <div className="relative overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-black/5 transition grayscale group-hover:grayscale-0 group-hover:-translate-y-1 group-hover:shadow-xl dark:bg-neutral-900 dark:ring-white/10">
                  { book.image && (
                    <Image src={book.image} alt={book.title} width={300} height={450} className="aspect-2/3 w-full object-cover" />
                  )}
                </div>
              ...
              <div className="mt-2 flex items-center gap-2 px-0.5" title={breakdown}>
                 <svg viewBox="0 0 20 20" fill="currentColor" className="h-4.5 w-4.5 text-amber-400">
